...
\begin{align}
    ...
    \end{cases}
     = h(t)
\end{align}
Let the Fourier Transform of $h(t)$ be given by $H(f)$,i.e
\begin{align}
    h(t)\fourier H(f)\\
...
\begin{align}
    H(f) = \int_{-\infty}^0 0\,dt + \int_0^T e^{-j2\pi f t}\,dt + \int_T^\infty 0\,dt\\
     = \frac{1 - e^{-j2\pi f T}}{j2\pi f}\\
      = \frac{1-\cos{2\pi f T} + j\sin{2\pi f T}}{j 2\pi f}\\
       = \frac{2\sin^2(\pi f T) + 2j\sin(\pi f T)\cos(\pi f T)}{j2 \pi f}\\
        = \frac{2\sin(\pi f T)}{2\pi f} \brak{\frac{\sin(\pi f T) + j\cos(\pi f t)}{j}}\\
         = T e^{-j \pi f T}\times \frac{\sin(\pi f T)}{\pi f T} \\
          = T e^{-j\pi f T} sinc(\pi f T)
\end{align}
